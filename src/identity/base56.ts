...
function assertSegment(value: string, label: string) {
  ...
  for (const character of value) {
    if (!SEMA_BASE56_ALPHABET.includes(character)) throw new Error(`Invalid SEMA Base56 character: ${character}`);
  }
...
export function parseInstallationId(value: string) {
  const installationId = value.trim();
  const segments = installationId.split('-');
  if (!installationId || segments.some((segment) => !segment)) throw new Error('InstallationID must contain non-empty hyphen-delimited segments');
  for (const segment of segments) assertSegment(segment, 'SEMA InstallationID segment');
  return installationId;
}

export function formatKeyId(input: { installationId: string; localValue: bigint }) {
...
export function parseKeyId(value: string): SemaKeyIdParts {
  const segments = value.trim().split('-');
  if (segments.length < 2 || segments.some((segment) => !segment)) throw new Error('A KeyID must contain an InstallationID and LocalID');
...
export function normalizeTypeCode(value: string) {
  ...
  if (!/^[A-Z][A-Z0-9_-]{0,31}$/.test(typeCode)) throw new Error('TypeCode must be 1-32 readable uppercase letters, digits, underscores, or hyphens');
  return typeCode;
}
